...
%INTERP1 1-D interpolation (table lookup) modified version to have all
%previous values equal to first known value and all after values as last
%known value
%
...
if ~isempty(extrapval)
    if ~isempty(Xq) && isfloat(Xq) && isreal(Xq)
        ...
        extptidsBefore=Xq < X(1);
        extptidsAfter=Xq > X(end);
        if any(extptids(:))
            Xq = Xq(~extptids);
        else
            extrapval = [];
...
if ~isempty(extrapval)
    ...
    Vq(~extptids,:) = VqLite;
    Vq(extptidsBefore,:)  = VqLite(1);
    Vq(extptidsAfter,:)  = VqLite(end);
