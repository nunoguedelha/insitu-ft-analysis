...
% clear all;
serialNumber='SN026';
e=1; %one for doing comparison on experiment1 data, 2 for experiment2
...
for i=3:6
    moreData.rawData.( ftNames{i})=[e1.dataset2.rawData.( ftNames{i})(1:(size(e1.dataset2.rawData.( ftNames{i}),1)/4),:);e2.dataset2.rawData.( ftNames{i})((size(e2.dataset2.rawData.( ftNames{i}),1)/2):end,:)];
    moreData.estimatedFtData.( ftNames{i})=[e1.dataset2.estimatedFtData.( ftNames{i})(1:(size(e1.dataset2.estimatedFtData.( ftNames{i}),1)/4),:);e2.dataset2.estimatedFtData.( ftNames{i})((size(e2.dataset2.estimatedFtData.( ftNames{i}),1)/2):end,:)];
end

[calibMatrices,offset,fullscale]=estimateMatrices(moreData.rawData,moreData.estimatedFtData);
