function [xmlStr,bestCMat,bestName,sCalibMat]=crossValidationDiffLambdas(serialNumbers,sensorName,experimentName,names2use)
...
cMat.(names2use{1}) = readCalibMat(strcat('../data/sensorCalibMatrices/matrix_',serialNumbers{1},'.txt'));

for i=2:length(serialNumbers)
        serialNumber=serialNumbers{i};

    cMat.(names2use{i+1}) = readCalibMat(strcat('../data/',experimentName,'/calibrationMatrices/',serialNumber));
...
if (index~=0)
    ...
    for i=1:length(names2use)
        ...
        ftDataNoOffset.(names2use{i})=removeOffset(reCalibData.(names2use{i}),data.(toCompareWith).estimatedFtData.(sensorsToAnalize));
            ...
            [inerOffset,~]=calculateOffset(sensorsToAnalize,data.(toCompareWith).inertial.ftData,data.(toCompareWith).inertial.estimatedFtData,data.(toCompareWith).cMat,cMat.(names2use{i}));
            for j=1:size(data.(toCompareWith).rawData.( sensorsToAnalize),1)
                reCalibData.(names2use{i})(j,:)=cMat.(names2use{i})*(data.(toCompareWith).rawData.( sensorsToAnalize)(j,:)');
